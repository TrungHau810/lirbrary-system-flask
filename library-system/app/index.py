...
@app.route("/")
def index():
    popular_books = dao.get_popular_books(8)
...
def books():
    ...
    data = dao.search_books(kw, cat, author, publisher)
...
def book_detail(book_id):
    b = dao.get_book(book_id)
    if not b:
        abort(404)

    related_books = []
    if b.category_id:
        related_books = dao.get_books_by_category(b.category_id, 4)
